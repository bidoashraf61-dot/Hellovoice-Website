...
(function () {
  ...
  /* Start a muted film, and keep trying until the browser lets it.
   *
   * WebKit refuses play() on a film that has not loaded its metadata yet —
   * NotAllowedError at readyState 0, measured on the iPhone engine at 570ms —
   * and accepts the same call a moment later. Chrome starts loading on play()
   * either way, which is why this only ever failed on iPhone: the hero sat on
   * its still and the background films never started. So a refusal fetches the
   * metadata and tries again when it arrives, and once more on the first touch,
   * which WebKit always allows. `want` lets a caller withdraw (the film scrolled
   * away) before a retry lands. */
  function playWhenAllowed(v, want) {
    var ok = function () { return !want || want(); };
    var q = v.play();
    if (!q || !q.catch) return;
    q.catch(function () {
      var retry = function () {
        if (!ok() || !v.paused) return;
        var r = v.play();
        if (r && r.catch) r.catch(function () {});
      };
      if (v.readyState >= 1) setTimeout(retry, 250);
      else {
        v.addEventListener("loadedmetadata", retry, { once: true });
        if (v.preload === "none") v.preload = "metadata";
        try { v.load(); } catch (err) {}
      }
      /* WebKit also refuses a film that is not visible yet — the home hero is
         transparent until its first frame and covered by the loading panel for
         the first second and a half — and a single retry at metadata landed
         inside that window too. So it is tried again once it can play, and on a
         short schedule that runs past the intro. Each retry is a no-op once the
         film is playing or the caller no longer wants it. */
      v.addEventListener("canplay", retry, { once: true });
      [800, 1700, 3000].forEach(function (ms) { setTimeout(retry, ms); });
      var first = function () {
        removeEventListener("touchstart", first);
        removeEventListener("pointerdown", first);
        retry();
      };
      addEventListener("touchstart", first, { passive: true });
  ...
  (function pageIntro() {
    ...
    /* Once per session. The full intro runs 7.5s, and every return to Home
       repeated it; a visitor who has seen it gets the page in its final state.
    ...
    /* On a phone it is compressed to about two seconds: the panel lifts at
       ...
       straight on. The desktop keeps the reference's rhythm. */
    ...
    if (small) {
      chars("[data-hero-title]", 1.1, 0.7, 0.25, "back.out(1.4)");
      chars("[data-hero-subtitle]", 1.3, 0.7, 0.25, "power3.out");
      chars("[data-hero-text]", 1.45, 0.6, 0.2, "power3.out");
    } else {
    ...
    if (social) tl.from(social, { opacity: 0, y: 30, duration: small ? 0.4 : 0.5, ease: "power1.out" }, small ? 1.7 : 4.89 + SHIFT);
    if (stat) tl.from(stat, { opacity: 0, y: 30, duration: small ? 0.4 : 0.5, ease: "power1.out" }, small ? 1.8 : 5.05 + SHIFT);
  ...
  /* Replaced 16 Sep 2026 (client comment #4): the six values are an arrow
   * slider with autoplay on every width, not a scroll-scrubbed row. The track
   * is a native scroll-snap row (so a thumb can swipe it and it still works
   * with no script at all); the arrows and the timer move it by one card. */
  function slider(root, track, opts) {
    opts = opts || {};
    var items = Array.prototype.slice.call(track.children);
    if (items.length < 2) return;
    var label = opts.label || "Slides";
    root.setAttribute("role", "region");
    root.setAttribute("aria-roledescription", "carousel");
    root.setAttribute("aria-label", label);
    if (!track.id) track.id = "slider-" + Math.random().toString(36).slice(2, 8);
    ...
    bar.className = "hv_slider_controls";
    var arrow = function (dir) {
      var b = document.createElement("button");
      b.type = "button";
      b.className = "hv_slider_arrow is-" + dir;
      ...
      b.innerHTML = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" ' +
        ...
        (dir === "prev" ? '<path d="M19 12H5M11 5l-7 7 7 7"/>' : '<path d="M5 12h14M13 5l7 7-7 7"/>') + '</svg>';
      return b;
    };
    var prev = arrow("prev"), next = arrow("next");
    var count = document.createElement("p");
    count.className = "hv_slider_count";
    count.setAttribute("aria-live", "polite");
    bar.appendChild(prev); bar.appendChild(count); bar.appendChild(next);
    (opts.controlsAfter || track).insertAdjacentElement("afterend", bar);

    function current() {
      var x = track.scrollLeft, best = 0, d = Infinity;
      items.forEach(function (it, i) {
        var dd = Math.abs(it.offsetLeft - items[0].offsetLeft - x);
        if (dd < d) { d = dd; best = i; }
      });
      return best;
    }
    ...
    function go(i) {
      var n = items.length;
      i = (i + n) % n;
      track.scrollTo({ left: items[i].offsetLeft - items[0].offsetLeft, behavior: reduced ? "auto" : "smooth" });
    }
    function paint() {
      var i = current();
      count.textContent = String(i + 1).padStart(2, "0") + " / " + String(items.length).padStart(2, "0");
    }
    prev.addEventListener("click", function () { hold(); go(current() - 1); });
    next.addEventListener("click", function () { hold(); go(atEnd() ? 0 : current() + 1); });
    var raf = 0;
    ...
    /* Autoplay: advances every few seconds while the slider is on screen,
       rests while a pointer or focus is on it and for a while after the
       visitor drives it by hand, and never runs under reduced motion. */
    var DELAY = opts.delay || 4500, timer = 0, visible = false, over = false, heldUntil = 0;
    function hold() { heldUntil = Date.now() + 9000; }
    function tick() {
      timer = 0;
      if (visible && !over && Date.now() >= heldUntil && !document.hidden) {
        go(atEnd() ? 0 : current() + 1);
      }
      schedule();
    }
    function schedule() { if (!timer && !reduced) timer = setTimeout(tick, DELAY); }
    root.addEventListener("pointerenter", function (e) { if (e.pointerType === "mouse") over = true; });
    root.addEventListener("pointerleave", function () { over = false; });
    root.addEventListener("focusin", function () { over = true; });
    root.addEventListener("focusout", function () { over = false; });
    track.addEventListener("pointerdown", hold, { passive: true });
    track.addEventListener("touchstart", hold, { passive: true });
    track.addEventListener("wheel", hold, { passive: true });
    if ("IntersectionObserver" in window) {
      new IntersectionObserver(function (en) { visible = en[0].isIntersecting; }, { threshold: 0.35 }).observe(track);
    } else { visible = true; }
    schedule();
  }

  (function valuesSlider() {
    $$(".about_track").forEach(function (track) {
      var row = $(".year_wrapper", track);
      if (row) slider(track, row, { label: "Our values", controlsAfter: $(".about_sticky", track) || row });
    });
  })();

  (function stepsSlider() {
    $$("[data-hv-slider]").forEach(function (root) {
      var row = $("[data-hv-slider-track]", root);
      if (row) slider(root, row, { label: root.getAttribute("data-hv-slider") || "Steps", delay: 5000 });
    });
  })();
  ...
  (function workStack() {
    ...
    /* Phones and tablets (client comment #5, 16 Sep 2026): the same idea as a
     * stack of cards. Each card is sticky (clone.css), so the next one slides
     * up over it; the one underneath sinks back as it is covered. */
    gsap.matchMedia().add("(max-width: 991px) and (prefers-reduced-motion: no-preference)", function () {
      var tweens = [];
      cards.slice(0, -1).forEach(function (card, k) {
        var nextCard = cards[k + 1];
        tweens.push(gsap.to(card, {
          scale: 0.92, ease: "none",
          scrollTrigger: {
            trigger: nextCard, start: "top bottom",
            end: function () { return "top " + (parseFloat(getComputedStyle(nextCard).top) || 90) + "px"; },
  ...
  (function logoFan() {
    ...
    /* Every card stays upright (client note, 16 Sep 2026). A card's angle is
     * its own slot plus the wrapper's turn; the logo inside is turned back by
     * the same amount so no brand is ever shown on its head. */
    var imgs = fan.map(function (el) { return $("img", el); });
    var visible = false;
    if ("IntersectionObserver" in window) {
      new IntersectionObserver(function (en) { visible = en[0].isIntersecting; }).observe(sec);
    }
    gsap.ticker.add(function () {
      if (!visible) return;
      var R = gsap.getProperty(wrap, "rotation") || 0;
      fan.forEach(function (el, i) {
        if (imgs[i]) gsap.set(imgs[i], { rotation: -((gsap.getProperty(el, "rotation") || 0) + R) });
      });
    });
  ...
  if (!reduced) {
    ...
    $$(".svc_tag_track").forEach(function (t, i) {
      var w = t.children[0].getBoundingClientRect().width;
      if (w) marquee(t, w, i % 2 ? 36 : -36);
    });

    $$(".leader_marquee_track").forEach(function (t) {
      var w = t.children[0].getBoundingClientRect().width;
      if (w) marquee(t, w, t.classList.contains("is-right") ? 40 : -40);
    });
  ...
  (function mediaSchedule() {
    ...
    /* The hero film has its own driver (heroFilm), and a film that starts itself
       under reduced motion is exactly what that preference asks us not to do —
       those visitors keep the posters, and nothing downloads. */
    var vids = $$("video").filter(function (v) {
      return !v.closest(".preloader") && !v.hasAttribute("data-hero-film");
    });
    if (vids.length) {
      var vio = new IntersectionObserver(function (entries) {
        entries.forEach(function (en) {
          var v = en.target;
          v.__inView = en.isIntersecting;
          if (en.isIntersecting && !reduced) {
            if (v.paused) playWhenAllowed(v, function () { return v.__inView; });
          } else if (!v.paused) {
            v.pause();
          }
        });
  ...
  $$("[data-button-primary], [data-button-primary-v2], .button_primary").forEach(function (btn) {
    ...
    hover(btn, function () {
      ...
      label.out();
      /* back to the stylesheet's own ground (lime since 16 Sep 2026), not a
         hard-coded transparent that would strip it after the first hover */
      gsap.to(btn, { clearProps: "backgroundColor", delay: 0.5 });
  ...
  (function menu() {
    ...
    /* Hidden while parked, not only moved. On iOS the page draws under the
       status bar while Safari's toolbars slide, and the parked panel's bottom
       edge — its email and phone links — showed up there on every page. */
    gsap.set(canvas, { y: 0, yPercent: -100, visibility: "hidden" });

    /* Parked off-screen it was still in the accessibility tree: VoiceOver swiped
       through its links on every page and Tab walked into them. inert takes the
       closed panel out of both; open, it is a modal dialog and Tab stays in it. */
    canvas.setAttribute("role", "dialog");
    canvas.setAttribute("aria-modal", "true");
    canvas.setAttribute("aria-label", "Site menu");
    canvas.inert = true;
    ...
    function setMenu(v) {
      ...
      canvas.inert = !v;
      if (v) gsap.set(canvas, { visibility: "visible" });
      gsap.to(canvas, { y: 0, yPercent: v ? 0 : -100, duration: 0.6, ease: e("inOutCubic"),
        onComplete: function () { if (!open) gsap.set(canvas, { visibility: "hidden" }); } });
      if (lines.length >= 3) {
    ...
    document.addEventListener("keydown", function (ev) {
      if (!open) return;
      if (ev.key === "Escape") { setMenu(false); burger.focus(); return; }
      if (ev.key !== "Tab") return;
      var items = [burger].concat($$("a[href], button, [role='button'], [tabindex]:not([tabindex='-1'])", canvas))
        .filter(function (el) { return el === burger || el.getClientRects().length; });
      var i = items.indexOf(document.activeElement);
      var next = ev.shiftKey ? (i <= 0 ? items.length - 1 : i - 1) : (i === items.length - 1 ? 0 : i + 1);
      ev.preventDefault();
      items[next].focus();
    });
  ...
  (function filmPlayer() {
    ...
    /* The frame takes the film's own shape (client comments #15/#16): the
       declared ratio first, then the real one — the file's own dimensions
       for a film we host, Vimeo's oEmbed for one of theirs. A film is never
       cropped: it fits the frame, and the frame fits the screen. */
    function shape(r) {
      if (!r || !isFinite(r) || r <= 0) return;
      frame.style.setProperty("--rn", r.toFixed(4));
      frame.classList.toggle("is-9-16", r < 1);
    }
    ...
    function open(src, ratio, title) {
      ...
      if (/^\/assets\/.*\.(mp4|webm)$/i.test(src)) {
        ...
        v.addEventListener("loadedmetadata", function () {
          if (v.videoWidth && v.videoHeight) shape(v.videoWidth / v.videoHeight);
        });
        frame.appendChild(v);
      } else {
        ...
        var vm = /player\.vimeo\.com\/video\/(\d+)(?:\?(?:.*&)?h=([0-9a-f]+))?/.exec(src);
        if (vm && window.fetch) {
          var page = "https://vimeo.com/" + vm[1] + (vm[2] ? "/" + vm[2] : "");
          fetch("https://vimeo.com/api/oembed.json?url=" + encodeURIComponent(page))
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (j) {
              if (j && j.width && j.height && frame.contains(iframe)) shape(j.width / j.height);
            })
            .catch(function () {});
        }
      }
  ...
  (function fullBleedFilm() {
    ...
    function want(on) {
      if (on) {
        if (!userPaused && vid.paused) playWhenAllowed(vid, function () { return !userPaused; });
      } else if (!vid.paused) {
        vid.pause();
      }
    }
  ...
  /* ------------------------------------------------------------- hero film
   * The hero character is an 8-second film.
   *
   * On a desktop the page is the transport: the section pins and the scroll
   * position scrubs the film, so every frame plays in its own order. Below
   * 992px there is no pin — on a phone, a screen and a half of scrolling that
   * moved nothing read as broken — and the film plays once where it stands and
   * rests on its last frame.
   *
   * Either way the still is the hero's own background (hero-exception.css) and
  ...
  (function heroFilm() {
    ...
    /* "playing" is the event WebKit sends. Waiting on loadeddata or seeked alone
       left iPhones blank: Safari fires neither for a paused film at frame 0
       (measured on iPhone 13 at 1s, 4s and 9s — opacity 0, readyState 1). */
    function markReady() { film.classList.add("is-ready"); }
    ["loadeddata", "seeked", "playing"].forEach(function (ev) {
      film.addEventListener(ev, markReady, { once: true });
    });
    if (film.readyState >= 2) markReady();

    /* A looping showreel since 16 Sep 2026 (client comment #1): no pin and no
       scrub on any width — the hero plays in place and the page scrolls on.
       Both routes in: the autoplay property, which WebKit acts on once the
       film is visible and has data, and play(), which playWhenAllowed retries
       on data, on timers and on the first touch. Low Power Mode refuses both
       until a touch; until then the still underneath is what shows. */
    film.loop = true;
    film.autoplay = true;
    playWhenAllowed(film);
    if ("IntersectionObserver" in window) {
      new IntersectionObserver(function (en) {
        if (en[0].isIntersecting) playWhenAllowed(film);
        else film.pause();
      }).observe(hero);
    }
  })();
  ...
  /* ------------------------------------------------------- back to the filter
   * The Work page runs 22-38 screens on a phone and its filter lives only at
   * the top. Once the filter is a screen out of sight a button offers the way
   * back to it, and focus lands on the first chip. */
  (function backToFilter() {
    var filter = $(".works_filter");
    if (!filter) return;
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "to_filter";
    btn.tabIndex = -1;
    btn.setAttribute("aria-hidden", "true");
    btn.setAttribute("aria-label", "Back to the filter");
    btn.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" ' +
      'stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M12 19V5M5 12l7-7 7 7"/></svg>';
    document.body.appendChild(btn);

    var shown = false, raf = 0;
    function check() {
      raf = 0;
      var past = filter.getBoundingClientRect().bottom < -window.innerHeight;
      if (past === shown) return;
      shown = past;
      btn.classList.toggle("is-shown", past);
      btn.tabIndex = past ? 0 : -1;
      if (past) btn.removeAttribute("aria-hidden"); else btn.setAttribute("aria-hidden", "true");
    }
    function queue() { if (!raf) raf = requestAnimationFrame(check); }
    addEventListener("scroll", queue, { passive: true });
    ...
    btn.addEventListener("click", function () {
      var y = Math.max(0, filter.getBoundingClientRect().top + window.scrollY - 90);
      if (lenis) lenis.scrollTo(y);
      else window.scrollTo({ top: y, behavior: reduced ? "auto" : "smooth" });
      var first = filter.querySelector("button");
      if (first) setTimeout(function () { first.focus({ preventScroll: true }); }, reduced ? 0 : 900);
    });
  })();
